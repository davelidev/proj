---
name: research-qc
description: Research and document high-performance trading strategies from the QuantConnect Community Forum. Use when the user needs to identify new alpha sources with CAGR >= 28% and Max Drawdown <= 58%.
---

# Research QuantConnect Forum

This skill automates the process of scouting the QuantConnect Community Forum for high-performance trading strategies and maintaining a structured research ledger.
...
3.  **Strategy Scouting**:
    - Search the QuantConnect Forum for strategies meeting the primary criteria:
        - **CAGR**: >= 28%
        - **Max Drawdown**: <= 58%
    - Focus on threads with attached code (Python) and validated backtest results.
...
5.  **Documentation & Pagination**:
    - Document results in `qc_forum_{x}.md`.
    - **Pagination Rule**: Each `qc_forum_{x}.md` file MUST contain exactly 50 strategy entries.

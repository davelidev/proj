...
class TQQQExpandingVix(QCAlgorithm):
    def Initialize(self):
    ...
    def OnData(self, data):
        ...
        # Kill switch based on VIX ratio
        if vix_ratio > 1.0:
            self.Liquidate(self.sym)
            return
        
        if not self.Portfolio.Invested:
            # Bull Market + Expanding Range + Trending (ADX>20) + Safe VIX Ratio
            if qqq_price > s200 and r1 > r2 and adx_val > 20 and vix_ratio < 1.0:

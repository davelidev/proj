...
| [52](#strategy-52)   | ✅    | Rotation        | 40%  | -40%  | 1.018  | 69    | 46    | 1.50     | 3.88         | 4/10   |
...
**Description:** **Note: cc1.json's name field for this strategy is stale.** The actual source code at `cc_037.py` is the same IBS extreme + 3-day max hold mean-reversion as Strategy 37 — it is *not* an '8 Leveraged ETF Permanent Rebalance' as the name suggests. The file was shared between two original metadata entries during cleanup; this strategy is functionally identical to Strategy 37.

*Overfit 5/10 — Identical code to Strategy 37 → identical rating. The metadata mismatch (name vs code) is a data-quality concern, not an overfit concern. Worth deleting this entry from cc1.json since it's a true duplicate.*
...
- **Symbols:** TQQQ
- **Rebalance:** Daily, 30 min after market open
- **⚠ Metadata note:** cc1.json `name` field says '8 Leveraged ETF EW Permanent + Monthly Rebalance' but the source code at cc_037.py is Strategy 37's IBS + 3-day hold logic. Either rename or drop the entry.
...
**Description:** **Note: cc1.json's name field for this strategy is stale.** The actual source code at `cc_043.py` is the QLD-based SMA(150) + IBS<0.05 hybrid from Strategy 43 — it is *not* a '5x 3x-Leveraged ETF Basket + QQQ Vol Gate'. File shared during cleanup; functionally identical to Strategy 43.

*Overfit 4/10 — Identical code to Strategy 43 → identical rating. The metadata mismatch is a catalog issue, not a code issue.*
...
- **Symbols:** QLD (execution); QQQ (signal)
- **Rebalance:** Daily, 30 min after market open
- **⚠ Metadata note:** cc1.json `name` field says '5x 3x-Leveraged ETF Basket + QQQ Vol Gate' but the source code at cc_043.py is Strategy 43's hybrid on QLD. Either rename or drop the entry.
...
**Description:** **Note: cc1.json's name field for this strategy is stale.** The actual source code at `cc_051.py` is a simple equal-weight monthly rebalance of the top 5 US stocks by market cap — there is *no* adaptive vol gate as the name suggests. The 'vol gate' from the original algos3 source was lost during file dedup.

*Overfit 2/10 — The actual code is a textbook monthly mega-cap basket — zero tuned parameters, dynamic universe. The 'adaptive vol gate' that would justify a higher overfit rating doesn't exist in this file.*

- **Universe:** Top 5 US stocks by market cap (all sectors, dynamic, monthly selection)
- **Allocation:** Equal weight (20% per name)
- **Entry:** None (permanent monthly rebalance)
- **Exit:** Per-name: liquidated when no longer in top 5
- **Rebalance:** Monthly (first trading day at 10:00 ET)
- **Symbols:** Dynamic — typically AAPL/MSFT/NVDA/GOOG/AMZN etc.
- **⚠ Metadata note:** cc1.json `name` field claims an 'Adaptive Vol Gate' but cc_051.py is just a monthly equal-weight rebalance with no vol gate. Original algos3 source was lost during file dedup.
...
**Description:** **Note: cc1.json's name field for this strategy is stale.** The actual source code at `cc_047.py` is the TQQQ self-SMA(200) + IBS<0.05 hybrid from Strategy 47 — there is *no* dynamic top-5 momentum sleeve or vol gate as the name claims. File shared during cleanup; functionally identical to Strategy 47.

*Overfit 5/10 — Identical code to Strategy 47 → identical rating. The metadata mismatch is a catalog issue, not a code issue.*
...
- **Symbols:** TQQQ (signal and execution)
- **Rebalance:** Daily, 30 min after market open
- **⚠ Metadata note:** cc1.json `name` field describes a top-5 momentum + TQQQ sleeve + vol gate strategy, but the source code at cc_047.py is Strategy 47's self-SMA200 + IBS hybrid. Either rename or drop the entry.

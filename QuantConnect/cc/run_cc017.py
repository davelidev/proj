"""cc017 batch runner — reads cc_algos/cc017_NNN.py, saves to backtests/backtest_cc017.json"""
...
def main():
    ...
    outpath    = os.path.join(script_dir, "backtests", "backtest_cc017.json")
    ...
    for n in nums:
        ...
        if key in results and "error" not in results[key] and results[key].get("status") == "completed":
            ...
            print(f"[resume] Skipping cc017_{n:03d} — {pf} CAGR={r['cagr']}, MaxDD={r['maxdd']}")
            continue
        if key in results:
            print(f"[retry] Re-running cc017_{n:03d} ({results[key].get('error','?')})")

        filepath = os.path.join(algos_dir, f"cc017_{n:03d}.py")
        if not os.path.exists(filepath):
            print(f"cc017_{n:03d}: not found, skipping")
            continue

        name = f"CC17_{n:03d}"
        print(f"\n{'='*60}\nRunning cc017_{n:03d}.py\n{'='*60}")
        sys.stdout.flush()

        result       = run_backtest(filepath, name)
        results[key] = result
        if "error" in result:
            print(f"\n  ❌ cc017_{n:03d}: {result['error']}")
        else:
            pf = "✅" if result["passed"] else "❌"
            print(f"\n  {pf} cc017_{n:03d}: CAGR={result['cagr']}, MaxDD={result['maxdd']}, Sharpe={result['sharpe']}")

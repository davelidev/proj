...
class Algo092(QCAlgorithm):
    """#092 — TQQQ regime switch with QUARTERLY (not monthly) basket weight rebalance."""
    ...
    def Initialize(self):
        ...
        self.basket = []; self.regime = "out"; self.qtr_seen = -1; self.weights = {}
    ...
    def R(self):
        ...
        cur_qtr = (self.Time.month - 1) // 3

        if nr != self.regime:
            self._liq()
            if nr == "tqqq": self.SetHoldings(self.tqqq, 1.0)
            elif nr == "basket":
                self.weights = self._w(); self.qtr_seen = cur_qtr
                for s, w in self.weights.items():
                    if w > 0: self.SetHoldings(s, w)
            self.regime = nr
            return

        if self.regime == "basket" and cur_qtr != self.qtr_seen:
            self.weights = self._w(); self.qtr_seen = cur_qtr

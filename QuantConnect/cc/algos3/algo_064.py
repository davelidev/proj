...
class Algo064(QCAlgorithm):
    """#064 — Top-7 mkt-cap top-3-of-7 by momentum + TQQQ vol gate."""
    ...
    def _w(self):
        ...
        if total <= 0: return {s: 1.0 / 3 for s, _ in ranked}
    ...
    def R(self):
        ...
        if on:
            ...
            for s, w in self.weights.items():
                ...
                if not self.in_market or abs(w - cur) > 0.05: self.SetHoldings(s, w)
            self.in_market = True
        else:
            if self.in_market:
                for s in list(self.Portfolio.Keys):

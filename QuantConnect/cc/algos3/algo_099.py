...
class Algo099(QCAlgorithm):
    """#099 — TQQQ + dyn top-7 momo with HARD daily exit on -8% TQQQ drop. Re-enter next day.
    Tail-risk overlay on the regime-switch design.
    """
    ...
    def _tqqq_today_ret(self):
        h = self.History(self.tqqq, 2, Resolution.Daily)
        if h.empty or len(h) < 2: return 0.0
        c = h['close'].values
    ...
    def R(self):
        ...
        # Hard tail-risk exit if TQQQ dropped >8% on last bar
        today_ret = self._tqqq_today_ret()
        if today_ret <= self.HARD_DROP and self.regime != "out":
            self._liq()
            self.regime = "out"
            self.in_cooldown = True
            return

        # If in cooldown, wait one day after a hard exit
        if self.in_cooldown:
            self.in_cooldown = False
            return

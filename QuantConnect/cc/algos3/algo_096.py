...
class Algo096(QCAlgorithm):
    """#096 — Regime switch with HALF-position TQQQ (50% TQQQ + 50% cash) in calm regime.
    Tests whether always-cash-buffer reduces DD enough to keep CAGR over 28%.
    """
    ...
    def R(self):
        ...
        elif v < self.CALM_VOL: nr = "half_tqqq"
        else: nr = "basket"

        if nr != self.regime:
            self._liq()
            if nr == "half_tqqq": self.SetHoldings(self.tqqq, 0.50)
            elif nr == "basket":

...
class Algo073(QCAlgorithm):
    """#073 — 100% TQQQ when vol<35% (very calm), else Top-7 dyn momo (50/50 with TQQQ)."""
    LOOKBACK = 63; CALM_VOL = 0.35; PANIC_VOL = 0.60; TOP_N = 7
    ...
    def R(self):
        ...
        if new_regime != self.regime:
            ...
            elif new_regime == "mixed":
                self.weights = self._w(); self.month_seen = self.Time.month
                for s, w in self.weights.items():
                    wt = w * 0.50
                    if wt > 0: self.SetHoldings(s, wt)
                self.SetHoldings(self.tqqq, 0.50)
            self.regime = new_regime
            return

        # Monthly rebalance for mixed regime
        if self.regime == "mixed" and self.Time.month != self.month_seen:
            self.weights = self._w(); self.month_seen = self.Time.month
            ...
            for s, w in self.weights.items():
                ...
                if abs(wt - cur) > 0.03 and wt > 0: self.SetHoldings(s, wt)
            cur_t = self.Portfolio[self.tqqq].HoldingsValue / max(1e-9, self.Portfolio.TotalPortfolioValue)
            if abs(0.50 - cur_t) > 0.03: self.SetHoldings(self.tqqq, 0.50)

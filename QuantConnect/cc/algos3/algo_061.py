...
class Algo061(QCAlgorithm):
    """#061 — Top-7 mkt-cap momo-weighted (3mo) + TQQQ vol<60% gate."""
    LOOKBACK = 63
    VOL_THRESH = 0.60
    TOP_N = 7
    ...
    def Initialize(self):
        ...
        self.basket = []
        self.in_market = False; self.month_seen = -1; self.weights = {}
    ...
    def R(self):
        ...
        if on:
            ...
            for s in list(self.Portfolio.Keys):
                if self.Portfolio[s].Invested and s != self.tqqq and s not in target_set:
                    self.Liquidate(s)

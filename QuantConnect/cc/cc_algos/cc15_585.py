...
class CC15_585(QCAlgorithm):
    def Initialize(self):
        ...
        self._macd=self.MACD("QQQ",12,26,9,MovingAverageType.Exponential,Resolution.Daily)
        self.SetWarmUp(50,Resolution.Daily); self._st=None; self.symbols=[]
    ...
    def Rebalance(self):
        if self.IsWarmingUp or not self._macd.IsReady or not self.symbols: return
        bullish=self._macd.Current.Value>self._macd.Signal.Current.Value
        if bullish:

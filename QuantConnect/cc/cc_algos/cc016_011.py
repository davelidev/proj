...
class CC16_661(QCAlgorithm):
    ...
    def _sd(self,h,n):
        k=[]; cl_list=[float(h['close'].iloc[-(i+1)]) for i in range(3)]
        for i in range(3):
            s=h.iloc[-(n+i):-i] if i>0 else h.iloc[-n:]
            hi=float(s['high'].max()); lo=float(s['low'].min()); c=cl_list[i]
            k.append((c-lo)/(hi-lo)*100 if hi>lo else 50)
        return sum(k)/3
    def Rebalance(self):
        if self.IsWarmingUp: return
        h=self.History(self.qqq,70,Resolution.Daily)
        if h.empty or len(h)<66: return
        d14=self._sd(h,14)
        st=1 if d14>50 else 0

...
class CC18_009(QCAlgorithm):
    def Initialize(self):
        self.SetStartDate(2014,1,1); self.SetEndDate(2025,12,31); self.SetCash(100000)
        self.q=self.AddEquity("QQQ",Resolution.Daily).Symbol
        self.t=self.AddEquity("TQQQ",Resolution.Daily).Symbol
        self.b=self.AddEquity("BIL",Resolution.Daily).Symbol
        self._ema=self.EMA(self.q,100,Resolution.Daily)
        self._st=None; self.SetWarmUp(120,Resolution.Daily)
    ...
    def R(self):
        if self.IsWarmingUp or not self._ema.IsReady: return
        s=self._slope(20)
        if s is None: return
        price=self.Securities[self.q].Price
        st=1 if s>0 and price>self._ema.Current.Value else 0

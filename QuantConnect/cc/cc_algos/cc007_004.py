...
class MorningStar(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        bullish_signal = (closes[-3]<opens[-3]) and (abs(closes[-2]-opens[-2]) < abs(closes[-3]-opens[-3])*0.3) and (closes[-1]>opens[-1]) and (closes[-1] > (opens[-3]+closes[-3])/2)

...
class CC16_699(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        hs=self.History(self.spy,25,Resolution.Daily)
        if hq.empty or len(hq)<22 or hs.empty or len(hs)<22: return
        rq=float(hq['close'].iloc[-1])/float(hq['close'].iloc[-21])-1
        rs=float(hs['close'].iloc[-1])/float(hs['close'].iloc[-21])-1
        # QQQ 20-day momentum stronger than SPY = tech leadership = bullish

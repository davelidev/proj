...
class UpDownVolRatio(QCAlgorithm):
    def Initialize(self):
    ...
    def Rebalance(self):
        ...
        # last 20 days: sum volume on up days / sum volume on down days
        up_v=sum(v[i] for i in range(-20,0) if c[i]>c[i-1])
        dn_v=sum(v[i] for i in range(-20,0) if c[i]<c[i-1])
        if dn_v<=0: return
        ratio = up_v / dn_v
        ratio_bull = ratio > 1.0
        if in_trend and ratio_bull: ns,wt,wb="BULL",1.0,0.0
        elif in_trend or ratio_bull: ns,wt,wb="MIXED",0.5,0.5

...
class ForceIndex13(QCAlgorithm):
    def Initialize(self):
    ...
    def Rebalance(self):
        ...
        # Force Index sum over last 13 days
        fi_sum=sum((c[i]-c[i-1])*v[i] for i in range(-13,0))
        fi_bull = fi_sum > 0
        if in_trend and fi_bull: ns,wt,wb="BULL",1.0,0.0
        elif in_trend or fi_bull: ns,wt,wb="MIXED",0.5,0.5

...
class CC16_727(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        # count days where close is in upper half of intraday range (buying pressure)
        high_close=sum(1 for i in range(len(h)-20,len(h))
                      if float(h['close'].iloc[i])>=(float(h['high'].iloc[i])+float(h['low'].iloc[i]))/2)
        st=1 if high_close>10 else 0

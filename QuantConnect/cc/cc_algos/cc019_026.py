...
class CC19_026(QCAlgorithm):
    def Initialize(self):
        self.SetStartDate(2014,1,1); self.SetEndDate(2025,12,31); self.SetCash(100000)
        self.q=self.AddEquity("QQQ",Resolution.Daily).Symbol
        self.t=self.AddEquity("TQQQ",Resolution.Daily).Symbol
        self.b=self.AddEquity("BIL",Resolution.Daily).Symbol
        self._sma=self.SMA(self.q,20,Resolution.Daily)
        self.thr=0.0; self.st=None; self.SetWarmUp(20+5,Resolution.Daily)
        self.Schedule.On(self.DateRules.EveryDay(self.q),self.TimeRules.AfterMarketOpen(self.q,30),self.R)
    def R(self):
        if self.IsWarmingUp or not self._sma.IsReady: return
        sma=self._sma.Current.Value
        if sma==0: return
        di=(self.Securities[self.q].Price-sma)/sma*100
        s=1 if di>self.thr else 0
        if s==self.st: return
        self.st=s

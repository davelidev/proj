...
class CC18_004(QCAlgorithm):
    def Initialize(self):
        ...
        self._ema=self.EMA(self.q,20,Resolution.Daily)
    ...
    def R(self):
        if self.IsWarmingUp or not self._psar.IsReady or not self._ema.IsReady: return
        price=self.Securities[self.q].Price
        st=1 if price>self._psar.Current.Value and price>self._ema.Current.Value else 0

...
class CC18_069(QCAlgorithm):
    def Initialize(self):
        ...
        self._ema={t:self.EMA(self.syms[t],13,Resolution.Daily) for t in self.tix}
    ...
    def R(self):
        ...
        for t in self.tix:
            ...
            lo=float(h["low"].values[-1])
            ema=self._ema[t].Current.Value
            bull_power=hi-ema
            bear_power=lo-ema
            if bull_power>0 and bear_power>-ema*0.02: bulls.append(t)
        n=len(bulls)

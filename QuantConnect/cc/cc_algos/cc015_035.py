...
class CC15_620(QCAlgorithm):
    def Initialize(self):
        ...
        self._kch=self.KCH("QQQ",20,2,MovingAverageType.Simple,Resolution.Daily)
        self.SetWarmUp(40,Resolution.Daily); self._st=None; self.symbols=[]
    ...
    def Rebalance(self):
        if self.IsWarmingUp or not self._kch.IsReady or not self.symbols: return
        ...
        if price>upper:
            ...
            self._st=1
        elif price<lower and self._st!=0:
            for sym in list(self.Portfolio.Keys):
                if sym in (self.qqq,self.bil): continue

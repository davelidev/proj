...
class ThreeState_DualROC2045_D200(QCAlgorithm):
    def Initialize(self):
        self.SetStartDate(2014, 1, 1); self.SetEndDate(2025, 12, 31); self.SetCash(100000)
        self.qqq=self.AddEquity("QQQ",Resolution.Daily).Symbol
        self.tqqq=self.AddEquity("TQQQ",Resolution.Daily).Symbol
        self.bil=self.AddEquity("BIL",Resolution.Daily).Symbol
        self.r1=self.ROC(self.qqq,20,Resolution.Daily); self.r2=self.ROC(self.qqq,45,Resolution.Daily)
    ...
    def Rebalance(self):
        if self.IsWarmingUp or not(self.r1.IsReady and self.r2.IsReady and self.hi.IsReady and self.lo.IsReady): return
        mid=(self.hi.Current.Value+self.lo.Current.Value)/2.0
        m=self.r1.Current.Value>0 and self.r2.Current.Value>0; d=self.Securities[self.qqq].Price>mid

...
class D3_M60_VOLCONT_UPVOL_15(QCAlgorithm):
    def Initialize(self):
    ...
    def Rebalance(self):
        ...
        try:
            f1 = c[-1]>c[-61]
            f2 = (lambda r=[c[i]/c[i-1]-1.0 for i in range(1,len(c))]: ((sum((x-sum(r[-10:])/10)**2 for x in r[-10:])/10)**0.5 < (sum((x-sum(r[-60:])/60)**2 for x in r[-60:])/60)**0.5))()
            f3 = (lambda up=sum(v[i] for i in range(-20,0) if c[i]>c[i-1]), dn=sum(v[i] for i in range(-20,0) if c[i]<c[i-1]): up>dn)()

...
class CC16_672(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        h=self.History(self.qqq,128,Resolution.Daily)
        if h.empty or len(h)<127: return
        roc126=float(h['close'].iloc[-1])/float(h['close'].iloc[0])-1
        st=1 if roc126>0 else 0

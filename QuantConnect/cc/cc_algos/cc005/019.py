...
class ThreeState_ROCD200_VIXPanic(QCAlgorithm):
    """3-state ROC(20)+D200 with VIX > 35 panic override → 100% BIL."""
    def Initialize(self):
    ...
    def Rebalance(self):
        if self.IsWarmingUp or not (self.roc.IsReady and self.hi200.IsReady and self.lo200.IsReady
                                    and self.Securities.ContainsKey(self.vix)):
            return
        vix_val = self.Securities[self.vix].Price
        if vix_val > 35:
            ns, wt, wb = "PANIC", 0.0, 1.0
        else:

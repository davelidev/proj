...
class ThreeDownDaysOversold(QCAlgorithm):
    def Initialize(self):
        self.SetStartDate(2014, 1, 1); self.SetEndDate(2025, 12, 31); self.SetCash(100000)
        self.qqq=self.AddEquity("QQQ",Resolution.Daily).Symbol
        self.tqqq=self.AddEquity("TQQQ",Resolution.Daily).Symbol
        self.bil=self.AddEquity("BIL",Resolution.Daily).Symbol
        self.closes=RollingWindow[float](5)
    ...
    def Rebalance(self):
        if self.IsWarmingUp or not(self.closes.IsReady and self.hi200.IsReady and self.lo200.IsReady and self.hi5.IsReady): return
        mid=(self.hi200.Current.Value+self.lo200.Current.Value)/2.0
        in_trend = self.Securities[self.qqq].Price > mid
        # 3 consecutive lower closes pattern
        c0,c1,c2,c3 = self.closes[0], self.closes[1], self.closes[2], self.closes[3]
        oversold = c0<c1 and c1<c2 and c2<c3
        price=self.Securities[self.qqq].Price
        if not self.Portfolio[self.tqqq].Invested:
            if in_trend and oversold:
                self.Liquidate(self.bil); self.SetHoldings(self.tqqq,1.0)
        else:
            if (not in_trend) or price >= self.hi5.Current.Value*0.999:

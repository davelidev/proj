...
class PSAR_Mom20_Top3(QCAlgorithm):
    def Initialize(self):
    ...
    def FineSelection(self, fine):
        ...
        return self.symbols
    def Rebalance(self):
        if self.IsWarmingUp or not self.psar.IsReady or not self.symbols: return
        ...
        m20 = c[-1] > c[-21]
        p_b = self.Securities[self.qqq].Price > self.psar.Current.Value
        n = int(in_trend)+int(m20)+int(p_b)

...
class CC19_050(QCAlgorithm):
    def Initialize(self):
        ...
        self.ef=self.EMA(self.q,8,Resolution.Daily)
        self.es=self.EMA(self.q,17,Resolution.Daily)
        self.thr=0.0; self.syms=[]; self.st=None; self.SetWarmUp(17+100,Resolution.Daily)
    ...
    def R(self):
        if self.IsWarmingUp or not self.ef.IsReady or not self.es.IsReady or not self.syms: return

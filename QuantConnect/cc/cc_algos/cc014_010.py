...
class CC14_CCI20_ADX14(QCAlgorithm):
    # CCI(20) + ADX(14): both bullish → TQQQ; either bearish → BIL; else top-5
    def Initialize(self):
    ...
    def Rebalance(self):
        if self.IsWarmingUp or not self._cci.IsReady or not self._adx.IsReady or not self.symbols: return
        ...
        adx_bull=pdi>ndi; adx_bear=self._adx.Current.Value>25 and ndi>pdi
        if cci>0 and adx_bull: st=2
        elif cci<-100 or adx_bear: st=0
        else: st=1

...
class CC16_723(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        path=sum(abs(closes[i]-closes[i-1]) for i in range(len(closes)-20,len(closes)))
        if path==0: return
        # signed efficiency ratio: direction × consistency > 0.1 = efficient uptrend
        ser=net/path
        st=1 if ser>0.1 else 0

...
class CC14_OBV_Dual(QCAlgorithm):
    # OBV(5) AND OBV(20) both up → TQQQ; OBV(5) down → BIL; mixed → top-5
    def Initialize(self):
    ...
    def _obv(self,n):
        ...
        o=[0.0]
        for i in range(1,len(c)): o.append(o[-1]+(v[i] if c[i]>c[i-1] else -v[i] if c[i]<c[i-1] else 0.0))
        return o[-1]>o[-n-1]
    def Rebalance(self):
        if self.IsWarmingUp or not self.symbols: return
        s5=self._obv(5); s20=self._obv(20)
        if s5 is None or s20 is None: return
        if s5 and s20: st=2
        elif not s5: st=0
        else: st=1

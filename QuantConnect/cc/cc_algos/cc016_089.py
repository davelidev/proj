...
class CC16_739(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        # count overnight gap-up days (open > previous close) in last 20
        gaps_up=sum(1 for i in range(len(h)-20,len(h))
                   if float(h['open'].iloc[i])>float(h['close'].iloc[i-1]))
        st=1 if gaps_up>7 else 0

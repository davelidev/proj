...
class OutsideBarHist(QCAlgorithm):
    def Initialize(self):
    ...
    def Rebalance(self):
        ...
        prev_h=float(h["high"].iloc[0]); prev_l=float(h["low"].iloc[0])
        cur_h=float(h["high"].iloc[1]); cur_l=float(h["low"].iloc[1])
        cur_o=float(h["open"].iloc[1]); cur_c=float(h["close"].iloc[1])
        outside_up = cur_h>prev_h and cur_l<prev_l and cur_c>cur_o

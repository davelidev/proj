...
class D_VR_Skew_T3_21(QCAlgorithm):
    def Initialize(self):
    ...
    def Rebalance(self):
        ...
        try:
            f1 = (lambda r=[c[i]/c[i-1]-1.0 for i in range(1,len(c))], r5=[c[i]/c[i-5]-1.0 for i in range(5,len(c))]: ((lambda m1=sum(r)/len(r), m5=sum(r5)/len(r5): ((lambda v1=sum((x-m1)**2 for x in r)/len(r), v5=sum((x-m5)**2 for x in r5)/len(r5): (v5/(5*v1) > 1.0 if v1>0 else False))()))()))()

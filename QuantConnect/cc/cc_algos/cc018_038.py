...
class CC18_038(QCAlgorithm):
    ...
    def _tsi(self,sym):
        ...
        if len(e2a)<2 or e2a[-1]==0: return None,None
        return 100*e2p[-1]/e2a[-1],100*e2p[-2]/e2a[-2] if e2a[-2]!=0 else None
    def R(self):
        if self.IsWarmingUp: return
        bulls=[]
        for t in self.tix:
            cur,prev=self._tsi(self.syms[t])
            if cur is not None and prev is not None and cur>0 and cur>prev:
                bulls.append(t)
        n=len(bulls)

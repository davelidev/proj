...
class D_LR_Skew_T3_23(QCAlgorithm):
    def Initialize(self):
    ...
    def Rebalance(self):
        ...
        try:
            f1 = (lambda ys=c[-50:], nL=50, xs=list(range(50)): ((lambda mx=sum(xs)/nL, my=sum(ys)/nL: ((lambda num=sum((xs[i]-mx)*(ys[i]-my) for i in range(nL)), den=sum((xs[i]-mx)**2 for i in range(nL)): (num/den > 0 if den>0 else False))()))()))()

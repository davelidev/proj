...
class CC16_709(QCAlgorithm):
    ...
    def Rebalance(self):
        ...
        h=self.History(self.ief,25,Resolution.Daily)
        if h.empty or len(h)<22: return
        roc20=float(h['close'].iloc[-1])/float(h['close'].iloc[-21])-1
        # 7-10yr bonds stable/rising = intermediate rates not spiking = healthy for growth
        st=1 if roc20>0 else 0

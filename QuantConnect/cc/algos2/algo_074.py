...
class Algo074(QCAlgorithm):
    """#074 — Top-3 mkt-cap concentrated momo + 50% TQQQ overlay + vol gate."""
    LOOKBACK = 63; VOL_THRESH = 0.60; TOP_N = 3
    BASKET_W = 0.50; TQQQ_W = 0.50
    ...
    def R(self):
        ...
        if on:
            ...
            for s, w in self.weights.items():
                ...
                if not self.in_market or abs(wt - cur) > 0.03:
                    if wt > 0: self.SetHoldings(s, wt)
            cur_t = self.Portfolio[self.tqqq].HoldingsValue / max(1e-9, self.Portfolio.TotalPortfolioValue)
            if not self.in_market or abs(self.TQQQ_W - cur_t) > 0.03: self.SetHoldings(self.tqqq, self.TQQQ_W)
            self.in_market = True
        else:
            if self.in_market:
                for s in list(self.Portfolio.Keys):

...
class Algo098(QCAlgorithm):
    """#098 — Regime switch using vol PERCENTILE rank against last 252 days.
    calm: vol < 30th percentile; panic: vol > 90th percentile; else basket.
    """
    LOOKBACK = 63; TOP_N = 7
    ...
    def R(self):
        ...
        if not self.vol_window.IsReady:
            nr = "tqqq" if v < 0.55 else ("basket" if v < 0.85 else "out")
        else:

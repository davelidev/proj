...
class Algo091(QCAlgorithm):
    """#091 — Regime switch with a 3-day vol confirmation. Avoid one-day vol spike whipsaws."""
    ...
    def Initialize(self):
        ...
        self.pending_regime = None; self.pending_count = 0
    ...
    def R(self):
        ...
        v = self._vol()
        if v is None: return
        if v >= self.PANIC_VOL: signal = "out"
        elif v < self.CALM_VOL: signal = "tqqq"
        else: signal = "basket"

        # Need CONFIRM_DAYS consecutive days of new signal before flipping
        if signal == self.regime:
            self.pending_regime = None; self.pending_count = 0
        elif signal == self.pending_regime:
            self.pending_count += 1
            if self.pending_count >= self.CONFIRM_DAYS:
                self._liq()
                if signal == "tqqq": self.SetHoldings(self.tqqq, 1.0)
                elif signal == "basket":
            ...
            self.pending_regime = signal; self.pending_count = 1
